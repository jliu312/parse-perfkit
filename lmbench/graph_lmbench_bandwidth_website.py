...
def graph(rw):
    ...
    plt.plot(logit(kvm2_df["Index"]),[y/1000 for y in kvm2_df["Bandwidth"]],label="Pixel 6a",color=colors[2],linewidth=4)
    plt.scatter(logit(kvm2_df["Index"]),[y/1000 for y in kvm2_df["Bandwidth"]],color=colors[2])    
...
#    plt.scatter(logit(native_df["Index"]),native_df["Bandwidth"],color=colors[0])
    ...
    plt.plot(logit(list(t4g2xl_df["Index"])[:-1]),[y/1000 for y in list(t4g2xl_df["Bandwidth"])[:-1]],label="t4g.2xlarge",color=colors[3],linewidth=4)
    plt.scatter(logit(list(t4g2xl_df["Index"])[:-1]),[y/1000 for y in list(t4g2xl_df["Bandwidth"])[:-1]],color=colors[3])
    
    print("t4g2xlarge:")
    print(statistics.mean(t4g2xl_df["Bandwidth"]))
    print("-------------------")
    
    plt.plot(logit(pix3_df["Index"]),[y/1000 for y in pix3_df["Bandwidth"]],label="Pixel 3a",color=colors[1],linewidth=4)
    plt.scatter(logit(pix3_df["Index"]),[y/1000 for y in pix3_df["Bandwidth"]],color=colors[1])
    ...
    plt.legend(ncol=3,loc="upper right",columnspacing=0.5)
    ax = plt.gca()
